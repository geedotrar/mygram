...
type UserQuery interface {
	...
	EditUser(ctx context.Context, id uint64, photo model.User) (model.User, error)
...
func (u *userQueryImpl) SignUp(ctx context.Context, user model.User) (model.User, error) {
	db := u.db.GetConnection()
	if err := db.
		WithContext(ctx).
		Table("users").
